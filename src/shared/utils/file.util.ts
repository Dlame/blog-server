...
export const uploadPath = path.resolve(__dirname, '../upload');
 ...
 * @param filePath 文件路径
 */
export function findOrCreateFilePath(filePath: string): Promise<boolean> {
 ...
 * @param filePath 文件路径
 */
export function decodeFile(filePath: string) {
...
//     - canvas
// ---
export function createFileContent({ title, content, createdAt, categories, tags }) {
  /**
   * 生成分类、标签字符串的方法
   * @param {Array} list
   * @return {String}
 ...
 */
export async function generateFile(article) {
  return new Promise((resolve, reject) => {
